...
const ElectronicsPage = () => {
  const shops = getShopsByCategory("Electronics");
  const allProducts = getAllProductsFromCategory("Electronics");
  ...
  const sortedProducts = [...allProducts].sort((a, b) => {
    ...
    if (sortBy === "rating") return 4.5 - 4.5; // Placeholder
    return 0;
  });
      ...
      <div className="bg-primary text-white py-12">
        <div className="container">
          ...
          <p className="text-lg opacity-90">
            Discover the latest electronics and gadgets from top stores
          </p>
        </div>
      </div>
      ...
      <div className="container py-6">
        ...
        {viewMode === "stores" && shops.length === 0 && (
          <div className="text-center py-12">
            <p className="text-secondary text-lg">No electronics stores found</p>
          </div>
        )}

        {viewMode === "products" && allProducts.length === 0 && (
          <div className="text-center py-12">
            <p className="text-secondary text-lg">No electronics products found</p>
          </div>
        )}
      </div>
    </div>
